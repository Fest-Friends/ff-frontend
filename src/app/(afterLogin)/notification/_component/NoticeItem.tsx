...
const NoticeItem = ({ time, user, isRead = false }: NoticeItemProps) => {
  return (
    <div className={`flex flex-col py-5 border-b border-areaBg ${isRead ? "opacity-40 pointer-events-none" : "opacity-100"}`}>
      ...
      <div className='flex justify-between items-center'>
        ...
        <div className='flex items-center gap-6 '>
          <p className='text-sm font-semibold text-textLess cursor-pointer'>거절하기</p>
          <p className='text-base font-semibold text-primary cursor-pointer'>수락하기</p>
        </div>
      </div>
    </div>
  );
